-- Fix accept_project_invite: digest() was not found because the function used
-- SET search_path = public, while Supabase installs pgcrypto in the extensions schema.
--
-- Token hashing must match Node.js:
--   createHash("sha256").update(plainToken).digest("hex")
-- i.e. SHA-256 over the UTF-8 bytes of the trimmed token string (not ::bytea cast).
