...
test.describe("deleteTask budget", () => {
  test("removes task costs from dashboard total but keeps loose expenses", async ({ page }) => {
    test.skip(!e2eCredentialsConfigured, "Set E2E_USER_EMAIL and E2E_USER_PASSWORD");
      const stamp = Date.now();
      const projectName = `E2E budget ${stamp}`;
      ...
      await page.getByLabel("Eigen geld").fill("50000");
      ...
      await page.goto("/dashboard/rooms?tab=overzicht");
      const taskRow = roomCard.locator("li").filter({ hasText: taskTitle });
      await taskRow.getByRole("button", { name: "Bewerken" }).click();
      await taskRow.getByRole("button", { name: "Verwijderen" }).click();
      await expect(roomCard.getByText(taskTitle)).not.toBeVisible({ timeout: 60_000 });

      await page.goto("/dashboard");
      await expectBudgetTotalSpent(page, 500);
      await expect(page.getByText(looseTitle)).toBeVisible();
  });
});
